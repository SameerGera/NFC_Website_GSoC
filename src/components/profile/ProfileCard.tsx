...
export default function ProfileCard({ member }: Props) {
  return (
    <div className="flex flex-col items-center gap-3 pt-6 pb-4">
      <div className="relative h-24 w-24 overflow-hidden rounded-full border-[3px] border-primary/30 bg-primary-bg">
        {member.profileImage ? (
          <img
      ...
      <div className="flex items-center gap-1.5">
        ...
        <svg className="h-5 w-5 text-verified" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="text-center text-sm text-text-secondary">
        <p>{member.department} Department, {member.year} Year • {member.role}</p>
        <p className="mt-0.5">{member.department} Web Team</p>
      </div>
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <a
          href={`mailto:${member.memberId.toLowerCase()}@gsock.tech`}
          className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary transition hover:bg-primary hover:text-white"
        >
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
